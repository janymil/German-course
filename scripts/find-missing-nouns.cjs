...
// Parse global words mapping manually
const regexWords = /words:\s*\{([\s\S]*?)\},\s*(chapters|export)/g;
let existingWords = [];
let match;
while ((match = regexWords.exec(content)) !== null) {
...
// Find ebook 02 sentences
const lines = content.split('\n');
let ch68text = [];
let inCh = false;
lines.forEach(line => {
    if (line.includes("title: 'Kapitel 1: Eine komische Halskette'")) inCh = true;
