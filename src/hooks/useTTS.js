...
export function useTTS() {
  ...
  const speak = useCallback((text, lang = 'de-DE', rate = 0.85, skipCleanup = false) => {
    ...
    let cleanLabel = text || '';
    if (!skipCleanup) {
      // If it looks like a full sentence (ends with punctuation), keep em-dashes, just remove pronunciation brackets.
      if (/[.!?]$/.test(cleanLabel.trim())) {
        cleanLabel = cleanLabel
          .replace(/\s*\(.*?\)/g, '')
          .replace(/\s*\[.*?\]/g, '')
          .trim();
      } else {
        // Vocabulary word/phrase: aggressively strip out everything after the em-dash or arrow
        cleanLabel = cleanLabel
          .replace(/\s*\(.*?\)/g, '')
  ...
  const speakStory = useCallback((sentences) => {
    ...
    return speak(fullText, 'de-DE', 0.85, true);
  }, [speak]);
